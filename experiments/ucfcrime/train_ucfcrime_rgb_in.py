...
flags.DEFINE_integer('num_frame_per_clib', 30, 'Nummber of frames per clib')
...
def run_training():
    ...
    if ckpt and ckpt.model_checkpoint_path:
        print("Loading checkpoint from %s" % ckpt.model_checkpoint_path)
        rgb_saver.restore(sess, ckpt.model_checkpoint_path)
        print("Load Successful")


    train_writer = tf.summary.FileWriter('./visual_logs/train_rgb_imagenet_10000_6_64_0.0001_decay', sess.graph)
    test_writer = tf.summary.FileWriter('./visual_logs/test_rgb_imagenet_10000_6_64_0.0001_decay', sess.graph)
    print("Begin")
    for step in xrange(FLAGS.max_steps):
        print("step %s start" %step)
        ...
        if (step+1) % 3000 == 0 or (step + 1) == FLAGS.max_steps:
            saver.save(sess, os.path.join(model_save_dir, 'i3d_ucfcrime_model_in'), global_step=step)
    print("done")
