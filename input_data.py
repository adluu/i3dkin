...
def get_data(filename, num_frames_per_clip, sample_rate, s_index=-1):
    ret_arr = []
    filenames = ''
    for filename in [filename]: #os.listdir(filename):
        filenames=sorted(os.listdir(filename), key=lambda a:int(a.split(".")[0]) ) 
        if len(filenames)==0:
            return [], s_index
        if (len(filenames)-s_index) <= num_frames_per_clip:
            filenames=(sorted( filenames, key=lambda a: int(a.split(".")[0]) ))
